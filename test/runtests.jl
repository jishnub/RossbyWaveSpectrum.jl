...
@testset "differential rotation" begin
    ...
    @testset "radial constant and constant" begin
        ...
        for colind in 1:3, rowind in 1:3
            if rowind == colind == 2
                @test_broken matrix_block(Mr, rowind, colind) ≈ matrix_block(Mc, rowind, colind) atol = 1e-10 rtol = 1e-1
            else
    ...
    # @testset "radial linear and linear" begin
    #     Mr = RossbyWaveSpectrum.differential_rotation_matrix(nr, nℓ, m,
    #         rotation_profile = :radial_linear; operators)
    #     Mc = RossbyWaveSpectrum.differential_rotation_matrix(nr, nℓ, m,
    #         rotation_profile = :linear; operators)
    #     for colind in 1:3, rowind in 1:3
    #         @test matrix_block(Mr, rowind, colind) ≈ matrix_block(Mc, rowind, colind) atol = 1e-10 rtol = 1e-2
    #     end
    # end

    # @testset "u × ω matrix for constant differential rotation" begin
    #     @testset "V" begin
    ...
    #             a::Float64 = 1 / (Δr / 2)
    #             anr = a * n * r
    #             4 / √15 * ΔΩ0::Float64 * 1 / r^2 * (-2Tn + anr * Unm1)
    #         end
    ...
    #     end
    # end
    @testset "u × ω matrix for linear differential rotation" begin
        @testset "V" begin
